context: herbal medicine, clinical herbalism, medicinal plants, herbalism, remedies, phytotherapy, holistic medicine, modern medicine, panacea, ills, facilitate the healing process, practitioner, healer, symptoms, syndromes, self-healing, ecological healing, 

herbalism: the study of herbs
herbs: whatever plant that heals

indian ayurveda
traditional chinese medicine
silamic unani medicine

homeopathy?

- classification
	- alphabetical? taxonomic? morphologic? therapeutic?
	- morphological:
		- organized drugs
			- leaves
			- flowers
			- fruits
			- seeds
			- aerial parts
			- whole plant
			- rhizomes
			- bark
			- roots
		- unorganized drugs
			- extracts
			- gums
			- resins
			- oils
			- fats
			- waxes

	- therapeutic
		- action-based
		- body system or organ affinity
		- medical system
			- ayurveda
			- western
			- traditional chinese medicine
		- biochemical
			- saponins
			- alkaloids
			- flavones
		- biogenetic
			- genetic taxonomy (little value to the phytotherapist)
		- geographical

- scientific name (latin binomial)
	- used to know exactly which herb is being prescribed
- common name
	
- taxonomy
	- kingdom (plantae)
	- division (if plants) phyla (if animals)
	- class
	- order
	- family
	- genus
	- species
		- ssp. subspecies
		- var. varieties
		- f. forms
		- x. hybrids of 2 plants of same genus
		- (L.) correted by ...
		- ICBN check?

	- you can add sub- or super- between ranks when useful
	- refer to apendix 7 for a list of ranks that contains useful medicinal plants
	
	- derivation of botanical names:
		- mythological
		- geographical / ecological
		- place
		- uses / properties
		- classical
		- people 
		- historical
		- anatomical 
		- color
		- aroma

- phytochemistry
	- constituents (active ingredients)
	- type of herbs
		- normalizers
		- effectors (for illnesses)
			- whole plant actions - mostly used in herbalism
			- effects of specific active chemical(s) - mostly used in pharmacology
	- ex constituents:
		- carbohydrates
		- glycosides
		- terpenes
		- phenolics
		- alkaloids
		
	- metabolites
		- primary (for plants survival)
		- secondary (more plant specific)
			- terpenes
			- phenolics
			- nitrogen-containing compounds
			
			- the dictionary of natural product (lists 170.000 compounds)
			
			- most important
				- terpenes
				- polyphenols
				- alkaloids

	- organic chemistry
		- molecules that contain carbon and hydrogen

- carbohydrates
