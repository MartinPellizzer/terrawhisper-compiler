...
vertices_plants = json_read('database/knowledge-graph/vertices-plants.json')
vertices_divisions = json_read('database/knowledge-graph/vertices-divisions.json')
vertices_classes = json_read('database/knowledge-graph/vertices-classes.json')
vertices_subclasses = json_read('database/knowledge-graph/vertices-subclasses.json')
vertices_orders = json_read('database/knowledge-graph/vertices-orders.json')
vertices_families = json_read('database/knowledge-graph/vertices-families.json')
    
edges_classes_divisions = json_read('database/knowledge-graph/edges-classes-divisions.json')
edges_subclasses_classes = json_read('database/knowledge-graph/edges-subclasses-classes.json')
edges_orders_subclasses = json_read('database/knowledge-graph/edges-orders-subclasses.json')
edges_families_orders = json_read('database/knowledge-graph/edges-families-orders.json')
...
def ai_paragraph_gen(filepath, data, obj, key, prompt, regen=False, print_prompt=False):
    ...
    if obj[key] == '':
        ...
        if reply.strip() != '':
            ...
            if 'couldn\'t' in reply: return
            if 'cannot' in reply: return
            obj[key] = reply
    ...
    for ailment_i, ailment in enumerate(ailments):
        ...
        if 0:
            for remedy_i, obj in enumerate(data['remedies']):
                if 'remedy_desc' in obj: obj['remedy_desc'] = ''
                json_write(json_filepath, data)
            continue
        ...
        if 0:
            if 'intro_desc' in data: data['intro_desc'] = ''
            json_write(json_filepath, data)
            continue
        ...
        # get related amazon affiliat links
        with open('assets/affiliates/amazon/amazon-affiliate-links.txt') as f: rows = f.read().strip().split('\n')
        random.shuffle(rows)

        row_random = rows.pop(0)
        affiliate_product_data = [x.strip() for x in row_random.split('\\')]
        ...
        article_html += f'<img class="article-img" src="{src}" alt="{alt}">\n'
        article_html += f'{util.text_format_1N1_html(data["intro_desc"])}\n'
        article_html += f'<p>This article explains in detail what are the best herbal teas for {ailment_name} and wh.</p>\n'
        if ads_manual:
            ad_html = f'''
                <div class="mb-48">
                    {GOOGLE_ADSENSE_DISPLAY_AD_SQUARE}
        ...
        for remedy_i, remedy in enumerate(data['remedies']):
            ...
            if preparation_name == 'teas':
                _remedy_name_scientific = remedy['herb_name_scientific']
                _remedy_slug = _remedy_name_scientific.strip().lower().replace(' ', '-').replace('.', '')
                _url = f'herbs/{_remedy_slug}'
                _json_filepath = f'database/json/{_url}.json'
                _data = json_read(_json_filepath)
                _herb_names_common = [x['name'] for x in _data['herb_names_common']]
                herb_name_common = _herb_names_common[0].lower().strip()
                ...
                # row_random = rows.pop(0)
                # affiliate_product_data = [x.strip() for x in row_random.split('\\')]
                # article_html += affiliate_gen(affiliate_product_data)
        ...
        if preparation_name == 'teas':
            row_random = rows.pop(0)
            affiliate_product_data = [x.strip() for x in row_random.split('\\')]
            _html = affiliate_gen(affiliate_product_data)
            article_html = article_html.replace('[aff]', _html)
    ...
    for obj_i, obj in enumerate(category_action[:4]):
        ...
        card_html = f'''
            <div>
                <a class="inline-block mb-48 no-underline" href="/herbs/{herb_slug}.html">
                    <img src="{src}">
                    <h2 class="mt-16 text-20 text-black">{headline.capitalize()}</h2>
                </a>
            </div>
        '''
        cards_html += card_html
...
def p_taxonomy():
    ...
    for vertex_order in vertices_orders:
        ...
        order_name = vertex_order['order_name']
        found = False
    ...
    html_article = ''
    html_article += f'<h2>Kingdoms</h2>\n'
    html_article += f'<ul>\n'
    html_article += f'<li>Animalia</li>\n'
    html_article += f'<li><a href="/herbs/taxonomy/plantae.html">Plantae</a></li>\n'
    html_article += f'<li>Fungi</li>\n'
    html_article += f'<li>Protista</li>\n'
    html_article += f'</ul>\n'
...
def p_taxonomy_kingdom(kingdom_slug):
    ...
    html_article = ''
    html_article += f'<h2>Divisions</h2>\n'
    html_article += f'<ul>\n'
    for division in json_article['divisions']:
        division_slug = division['division_slug']
        html_article += f'<li><a href="/herbs/taxonomy/{kingdom_slug}/{division_slug}.html">{division_slug}</a></li>\n'
    html_article += f'</ul>\n'
...
def p_taxonomy_division(vertex_division):
    ...
    html_article = ''
    html_article += f'<h2>Classes</h2>\n'
    html_article += f'<ul>\n'
    for _class in json_article['classes']:
        class_slug = _class['class_slug']
        html_article += f'<li><a href="/herbs/taxonomy/{kingdom_slug}/{division_slug}/{class_slug}.html">{class_slug}</a></li>\n'
    html_article += f'</ul>\n'
...
def p_taxonomy_class(vertex_class):
    ...
    html_article = ''
    html_article += f'<h2>Subclasses</h2>\n'
    html_article += f'<ul>\n'
    for subclass in json_article['subclasses']:
        subclass_slug = subclass['subclass_slug']
        html_article += f'<li><a href="/herbs/taxonomy/{kingdom_slug}/{division_slug}/{class_slug}/{subclass_slug}.html">{subclass_slug}</a></li>\n'
    html_article += f'</ul>\n'
...
def p_taxonomy_subclass(vertex_subclass):
    ...
    html_article = ''
    html_article += f'<h2>Orders</h2>\n'
    html_article += f'<ul>\n'
    for order in json_article['orders']:
        order_slug = order['order_slug']
        html_article += f'<li><a href="/herbs/taxonomy/{kingdom_slug}/{division_slug}/{class_slug}/{subclass_slug}/{order_slug}.html">{order_slug}</a></li>\n'
    html_article += f'</ul>\n'
...
def p_taxonomy_order(vertex_order):
    ...
    html_article = ''
    html_article += f'<h2>Families</h2>\n'
    html_article += f'<ul>\n'
    for family in json_article['families']:
        family_slug = family['family_slug']
        html_article += f'<li><a href="/herbs/taxonomy/{kingdom_slug}/{division_slug}/{class_slug}/{subclass_slug}/{order_slug}/{family_slug}.html">{family_slug}</a></li>\n'
    html_article += f'</ul>\n'
...
if 0:
    ...
    for vertex_family in vertices_families:
        p_taxonomy_family(vertex_family)
    '''
    p_taxonomy_families()
    quit()
