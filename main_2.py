...
def articles_preparations(preparation_slug):
    ...
    for ailment_i, ailment in enumerate(ailments):
            ...
            if data[key] == '':
                ...
                for _ in range(20):
                    prompt = f'''
                        ...
                        Relply in JSON format by extracting the format from the example below:
                        [
                            {{"herb_name": "insert herb name here", "confidence_score": insert confidence score number here}},
                            {{"herb_name": "insert herb name here", "confidence_score": insert confidence score number here}},
                            {{"herb_name": "insert herb name here", "confidence_score": insert confidence score number here}}
                        ]
                    ...
                    if _data != '':
                        _remedies = []
                        for _obj in _data:
                            if 'herb_name' not in _obj: continue
                            if 'confidence_score' not in _obj: continue
                            _remedies.append(_obj)
                        if _remedies != []:
                            for _remedy in _remedies:
                                found = False
                                for _output in _outputs:
                                    if _remedy['herb_name'].strip().lower() == _output['herb_name']:
                                        _output['confidence_score'] += int(_remedy['confidence_score'])
                                        found = True
                                        break
                                if not found:
                                    _outputs.append({
                                        'herb_name': _remedy['herb_name'].strip().lower(),
                                        'confidence_score': _remedy['confidence_score'],
                                    })
                _outputs = sorted(_outputs, key=lambda x: x['confidence_score'], reverse=True)
                data[key] = _outputs[:remedies_num]
                json_write(json_filepath, data)
        ...
        for remedy_i, remedy in enumerate(data['remedies'][:remedies_num]):
            article_html += f'<h3>{remedy_i+1}. {remedy["herb_name_scientific"].title()}</h3>'
            article_html += f'<p>{remedy["remedy_desc"]}</p>'

        head_html = head_html_generate(title)
        html = f'''
            <!DOCTYPE html>
            <html lang="en">
            {head_html}
...
def page_home():
    head_html = head_html_generate('the future of wellness is natural remedies')
    section_1 = f'''
        <section class="container-xl grid-container mb-48">
            <a class="no-underline bg-center bg-cover card-wide card-tall flex items-end pl-16 pb-16 pr-48" href="/remedies/integumentary-system/dry-hair/teas.html" style="background-image: linear-gradient(rgba(0, 0, 0, 0.0), rgba(0, 0, 0, 0.5)), url(/images-static/benefits.png)">
                <div>
                    <span class="inline-block text-12 text-white bg-black uppercase mb-16 pl-8 pr-8 pt-4 pb-4">
                        AMBIENTE
                    </span>
                    <h2 class="text-white text-24 mb-16">
                        L'ozonizzazione catalitica rivoluziona la rimozione delle emmissioni industriali di NOx.
                    </h2>
                    <p class="text-white">
            ...
            <a class="no-underline bg-center bg-cover card-wide flex items-end pl-16 pb-16 pr-48" href="/remedies/integumentary-system/dry-hair/teas.html" style="background-image: linear-gradient(rgba(0, 0, 0, 0.0), rgba(0, 0, 0, 0.5)), url(/images-static/benefits.png)">
                <div>
                    <span class="inline-block text-12 text-white bg-black uppercase mb-16 pl-8 pr-8 pt-4 pb-4">
                        LAVORAZIONE
                    </span>
            ...
            <a class="no-underline bg-center bg-cover flex items-end pl-16 pb-16 pr-48" href="/remedies/integumentary-system/dry-hair/teas.html" style="background-image: linear-gradient(rgba(0, 0, 0, 0.0), rgba(0, 0, 0, 0.5)), url(/images-static/benefits.png)">
                <div>
                    <span class="inline-block text-12 text-white bg-black uppercase mb-16 pl-8 pr-8 pt-4 pb-4">
                        AMBIENTE
                    </span>
