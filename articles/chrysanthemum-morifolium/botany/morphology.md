# Chrysanthemum morifolium morphology

![Chrysanthemum morifolium morphology](/images/chrysanthemum-morifolium-botany-morphology.jpg "Chrysanthemum morifolium morphology")

Chrysanthemum morifolium, commonly known as chrysanthemum or mums, features a fibrous root system characterized by a taproot structure. These roots penetrate the soil effectively, reaching a depth of 15-30 cm (6-12 inches) with a diameter ranging from 0.2 to 0.5 cm (0.08-0.2 inches). They are whitish or light brown, smooth, and hairless, lacking nodules but equipped with fine root hairs to increase absorption.

The herbaceous stems of Chrysanthemum morifolium are upright, cylindrical, and range from 30-60 cm (11.8-23.6 inches) in height with a diameter of 0.5-1 cm (0.2-0.4 inches). They are green, smooth, and glabrous, featuring nodes and internodes spaced 5-10 cm (2-4 inches) apart.

Chrysanthemum leaves are simple, variable in shape, and alternate, measuring 5-15 cm (2-6 inches) in length and 2-7 cm (0.8-2.8 inches) in width. They have a smooth, slightly waxy surface, an acute apex, cuneate or rounded base, dentate or serrate margins, and pinnate venation. Leaves are petiolate, with a 2-8 cm (0.8-3.2 inches) long petiole, and stipules are absent.

The inflorescence type is a capitulum, featuring individual, non-fused, disc-shaped flowers with a standardized size of 2.5 cm (0.98 inches). Flowers are smooth, composed of 21 floral parts, lack sepals, and have numerous stamens. Chrysanthemum pistils are single with an inferior ovary, a style, and two stigma structures. Stipules are absent, and the flowers are supported by a 30 cm (11.81 inches) long peduncle with 2 cm (0.79 inches) pedicels at the base.

Chrysanthemum fruits are achenes, clustered, and sessile, with an oblong shape measuring 2.5 cm (0.98 inches) in length and 1 mm (0.04 inches) in width and thickness. They are brown or black when mature, have a smooth and glossy surface, and do not exhibit dehiscence.

The seeds are simple, oval, and arranged singly within the flower head. Each head typically contains 10-15 seeds measuring 2.5-3 mm (0.1-0.12 inches) in length, 1.5-2 mm (0.06-0.08 inches) in width, and 0.5-1 mm (0.02-0.04 inches) in thickness. They have a hard, brown or black seed coat, lack a visible hilum, and feature a straight embryo with two fleshy cotyledons and a tiny radicle.

In this article you will learn about the morphology of Chrysanthemum morifolium by analyzing the main parts of this plant and their characteristics.

## Roots



Chrysanthemum morifolium has a fibrous root system, which means it consists of a network of fine roots rather than a single dominant root. The root structure is taproot, characterized by a primary, thick root from which smaller roots branch out in a dichotomous pattern, forming a tree-like structure.

The root tips are acute and pointed, enabling them to penetrate the soil effectively. Chrysanthemum morifolium roots have a high density, with numerous fine roots extending from the taproot. These roots typically grow to a depth of 15-30 cm (6-12 inches) in the soil, allowing the plant to access water and nutrients efficiently.

In terms of diameter, Chrysanthemum morifolium roots range from 0.2 to 0.5 cm (0.08-0.2 inches), and their length can reach up to 30 cm (12 inches). The roots appear whitish or light brown in color and have a smooth and glabrous surface texture, which aids in the absorption of water and minerals from the soil.

Notably, Chrysanthemum morifolium roots lack nodules, which are specialized structures found in some plants for nitrogen fixation. However, they are equipped with fine root hairs that are numerous and assist in increasing the root's surface area for absorption.

The following table shows in detail the morphological characteristics of Chrysanthemum morifolium roots.

| Characteristic | Description |
| --- | --- |
| Root System Type  | Fibrous |
| Root Structure  | Taproot |
| Root Branching  | Dichotomous |
| Root Tips  | Acute, pointed |
| Root Density  | High, numerous |
| Root Depth  | 15-30 cm (6-12 inches) |
| Root Diameter  | 0.2-0.5 cm (0.08-0.2 inches) |
| Root Length  | Up to 30 cm (12 inches) |
| Root Color  | Whitish or light brown |
| Root Surface Texture  | Smooth and glabrous |
| Root Nodules  | Absent |
| Root Hairs  | Present, fine and numerous |

![none](/images/chrysanthemum-morifolium-botany-morphology-roots.jpg "none")

## Stems



The stem type of Chrysanthemum morifolium is herbaceous, indicating that it is non-woody. This characteristic is typical of many herbaceous plants, providing flexibility and support during growth. Stem arrangement in this species is opposite, meaning that leaves are attached in pairs along the stem. This opposite arrangement creates a balanced and symmetric appearance in the plant's foliage.

Chrysanthemum morifolium stems are primarily upright and cylindrical in shape. This characteristic contributes to the plant's overall growth habit and allows it to reach a height of 30-60 cm (11.8-23.6 inches). The stem height can vary within this range, depending on the specific cultivar and growing conditions. The stem diameter typically ranges from 0.5-1 cm (0.2-0.4 inches), providing adequate structural support for the plant.

The stem color is uniformly green, reflecting the presence of chlorophyll and the plant's photosynthetic activity. This green coloration is characteristic of healthy Chrysanthemum morifolium stems. Stem texture is smooth and slightly ribbed, contributing to the overall aesthetics of the plant. The smooth texture enhances the visual appeal of the stems.

The stem surface is glabrous, meaning it is hairless. This feature is consistent with many herbaceous plants, where the stem's smooth surface aids in efficient nutrient transport. Stem nodes are present along the stem, corresponding to the points where leaves attach. These nodes play a vital role in leaf and flower development.

Stem internodes are relatively spaced, typically measuring about 5-10 cm (2-4 inches) apart. These internodes allow for proper spacing between leaves and facilitate healthy growth.

The following table shows in detail the morphological characteristics of Chrysanthemum morifolium stems.

| Characteristic | Description |
| --- | --- |
| Stem Type  | Herbaceous (non-woody) |
| Stem Arrangement  | Opposite (leaves attached in pairs) |
| Stem Shape  | Upright and cylindrical |
| Stem Height  | 30-60 cm (11.8-23.6 inches) |
| Stem Diameter  | 0.5-1 cm (0.2-0.4 inches) |
| Stem Color  | Green |
| Stem Texture  | Smooth and slightly ribbed |
| Stem Surface  | Glabrous (hairless) |
| Stem Nodes  | Present, where leaves attach to the stem |
| Stem Internodes  | About 5-10 cm (2-4 inches) apart |

## Leaves



, commonly known as chrysanthemum or mums, is a popular ornamental plant with a range of distinctive leaf characteristics. The leaves of Chrysanthemum morifolium are simple, typically variable in shape, and alternate in arrangement along the stems. The orientation of the leaves is erect, giving the plant an upright appearance.

The leaves of Chrysanthemum morifolium exhibit variations in size, with lengths ranging from 5 to 15 centimeters (2 to 6 inches) and widths spanning 2 to 7 centimeters (0.8 to 2.8 inches). In terms of thickness, the leaves measure approximately 0.2 to 0.5 centimeters (0.08 to 0.2 inches). Their color is primarily green, although it can vary among different cultivars.

These leaves have a smooth texture and a slightly waxy surface, lending them a shiny appearance. At the leaf apex, they come to a point, with an acute shape, while the leaf base is often cuneate or rounded. The margins of the leaves are characterized by dentate or serrate edges. The venation is pinnate, and the leaf lamina is simple and entire.

A prominent midrib runs along the length of the leaf, adding structural support. The leaves are petiolate, meaning they are attached to the stem by a petiole, which typically measures 2 to 8 centimeters (0.8 to 3.2 inches) in length. Stipules, small leaf-like structures at the base of the petiole, are typically absent in Chrysanthemum morifolium.

The following table shows in detail the morphological characteristics of Chrysanthemum morifolium leaves.

| Characteristic | Description |
| --- | --- |
| Leaf Complexity  | Simple |
| Leaf Shape  | Variable (e.g., ovate, lanceolate) |
| Leaf Arrangement  | Alternate |
| Leaf Orientation  | Erect |
| Leaf Length  | 5-15 cm (2-6 inches) |
| Leaf Width  | 2-7 cm (0.8-2.8 inches) |
| Leaf Thickness  | 0.2-0.5 cm (0.08-0.2 inches) |
| Leaf Color  | Green (may vary with cultivar) |
| Leaf Texture  | Smooth, glabrous |
| Leaf Surface  | Shiny, slightly waxy |
| Leaf Apex  | Acute, pointed |
| Leaf Base  | Cuneate, rounded |
| Leaf Margin  | Dentate, serrate |
| Leaf Venation  | Pinnate |
| Leaf Lamina  | Simple, entire |
| Leaf Midrib  | Prominent |
| Leaf Attachment  | Petiolate (stalked) |
| Leaf Petiole  | 2-8 cm (0.8-3.2 inches) |
| Leaf Stipule | Absent |

## Flowers



The inflorescence type of Chrysanthemum morifolium is known as a capitulum, which consists of multiple individual flowers clustered together. These flowers exhibit a simple and non-complex structure, boasting a radial or actinomorphic symmetry, meaning they are symmetrical in all directions. 

Each chrysanthemum flower is a single, non-fused structure, attached to the stem in a solitary manner, with one flower per stem. The flowers take on a disc-shaped form, with a standardized size of approximately 2.5 centimeters (0.98 inches) in both length and width. Chrysanthemum morifolium comes in a range of colors, making it a versatile and visually appealing plant for ornamental purposes.

When touched, the flowers feel smooth to the touch, as they have a glabrous or hairless texture. These flowers are composed of 21 floral parts, which may vary slightly between different chrysanthemum cultivars. Interestingly, they lack sepals, as the colorful petals replace this protective outer layer. The combined structure of sepals and petals is referred to as the perianth.

Within the flower, numerous stamens can be found, although the exact count may vary. Chrysanthemum morifolium typically features a single pistil with an inferior ovary, positioned below other floral parts. The pistil is characterized by a single style and two stigma structures at its tip. Stipules, small leaf-like structures found at the base of some flowers, are absent in chrysanthemum.

The flowers are supported by a peduncle, a stalk-like structure that can grow to a length of approximately 30 centimeters (11.81 inches). At the base of each flower, there is a pedicel, which measures around 2 centimeters (0.79 inches) in length.

The following table shows in detail the morphological characteristics of Chrysanthemum morifolium flowers.

| Characteristic | Description |
| --- | --- |
| Flower Type  | Capitulum |
| Inflorescence Type  | Simple (non-complex) |
| Flower Complexity  | Radial (actinomorphic) |
| Flower Symmetry  | Single (non-fused) |
| Flower Structure   | Solitary (single flower per stem) |
| Flower Attachment  | Disc-shaped |
| Flower Shape  | 2.5 cm (0.98 inches) |
| Flower Length  | 2.5 cm (0.98 inches) |
| Flower Width  | Various colors (specify if needed) |
| Flower Thickness  | Smooth |
| Flower Color  | Glabrous (hairless) |
| Number Of Floral Parts  | 21 (specify details of each part if needed) |
| Flower Sepals   | 0 (petals replace sepals) |
| Flower Petals   | 21 (petal count may vary) |
| Flower Perianth  | 21 (perianth includes sepals and petals) |
| Flower Stamen  | Numerous (specify exact count if known) |
| Flower Pistil  | Single |
| Flower Ovary  | Inferior (below other floral parts) |
| Flower Style  | 1 (stalk-like structure) |
| Flower Stigma  | 2 (at the tip of the style) |
| Flower Stipules  | Absent |
| Flower Bracts  | Small and inconspicuous |
| Flower Peduncle  | 30 cm (11.81 inches) long |
| Flowers Type | 2 cm (0.79 inches) long |

## Fruits



The fruits of Chrysanthemum morifolium are classified as achenes, a type of dry, one-seeded fruit. They are typically found in clustered arrangements, densely packed together, attached directly to the receptacle without stalks. The fruit attachment is sessile, meaning they have no stalk or stem connecting them to the plant. 

In terms of shape, chrysanthemum fruits are oblong and cylindrical, with a length of approximately 2.5 cm (0.98 inches) and a relatively small width and thickness of about 1 mm (0.04 inches). When mature, these fruits take on a brown or black color. The surface of chrysanthemum fruits is smooth and glossy, with no prominent markings.

One distinctive feature of chrysanthemum fruits is their dehiscence, or rather, their lack of it. These fruits are indehiscent, meaning they do not split open when they reach maturity. This characteristic sets them apart from other types of fruits that exhibit dehiscence.

The following table shows in detail the morphological characteristics of Chrysanthemum morifolium fruits.

| Characteristic | Description |
| --- | --- |
| Fruit Type  | Achene |
| Fruit Arrangement  | Clustered |
| Fruit Distribution  | Dense, attached to the receptacle |
| Attachment | Sessile |
| Fruit Shape  | Oblong, cylindrical |
| Fruit Length  | 2.5 cm (0.98 inches) |
| Fruit Width  | 1 mm (0.04 inches) |
| Fruit Thickness  | 1 mm (0.04 inches) |
| Fruit Color  | Brown or black when mature |
| Fruit Texture  | Smooth |
| Fruit Surface  | Glossy |
| Fruit Marking  | No prominent markings |
| Fruit Dehiscence | Indehiscent (does not split open when ripe) |

## Seeds



The seeds of Chrysanthemum morifolium are simple, arranged singly within the flower head. They have an oval shape and lack appendages. 

Each chrysanthemum head typically contains a variable number of seeds, usually ranging from 10 to 15. These seeds are relatively small, measuring about 2.5 to 3 millimeters in length, 1.5 to 2 millimeters in width, and 0.5 to 1 millimeter in thickness. They are hard and exhibit a brown or black seed coat, which is smooth and shiny. The seed coat is quite thin, measuring around 0.1 to 0.2 millimeters in thickness, and is impermeable.

Chrysanthemum morifolium seeds lack a visible hilum, and their embryos are straight. Inside the seed, two fleshy cotyledons are present, along with a small radicle.


The following table shows in detail the morphological characteristics of Chrysanthemum morifolium seeds.

| Characteristic | Description |
| --- | --- |
| Seed Type  | Simple |
| Seed Arrangement  | Single |
| Seed Shape  | Oval |
| Seed Appendages  | Absent |
| Seed Number  | Variable, typically 10-15 seeds per head |
| Seed Length  | 2.5-3 mm (0.1-0.12 inches) |
| Seed Width  | 1.5-2 mm (0.06-0.08 inches) |
| Seed Thickness  | 0.5-1 mm (0.02-0.04 inches) |
| Seed Hardness  | Hard |
| Seed Coat Color  | Brown or black |
| Seed Coat Surface  | Smooth |
| Seed Coat Texture  | Shiny |
| Seed Coat Thickness  | 0.1-0.2 mm (0.004-0.008 inches) |
| Seed Coat Permeability  | Impermeable |
| Seed Hilum  | Absent |
| Seed Embryo  | Straight |
| Seed Cotyledon | Two, fleshy |
| Seed Radicle | Present, tiny |

