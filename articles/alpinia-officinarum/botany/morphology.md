# Alpinia officinarum morphology

![Alpinia officinarum morphology](/images/alpinia-officinarum-botany-morphology.jpg "Alpinia officinarum morphology")

Alpinia officinarum, commonly known as Lesser Galangal, is characterized by its unique plant features. The plant has a fibrous root system consisting of a main taproot and lateral fibrous roots with dichotomous branching. The main taproot typically reaches a depth of 20-30 centimeters and has a diameter of 1.5 centimeters, while the lateral roots are finer. These light brown roots have a smooth, hairy texture, enabling nutrient and water absorption, without developing nodules.

The herbaceous stems of Alpinia officinarum are cylindrical and can grow from 1 to 3 meters in height, with alternating leaf arrangement. These green stems have a smooth, slightly ribbed surface, often marked with reddish or brown patches. Stem nodes are distinct, where leaves and branches originate, with internodes between them.

The rhizomes of the plant are arranged sympodially and are cylindrical, ranging from 20 to 60 centimeters in height and 1 to 3 centimeters in diameter. Their surface is smooth, and they have 1 to 3 nodes along with 5 to 15 centimeter internodes.

The leaves are oblong, simple, and alternately arranged, with parallel venation, no stipules, and a glossy, green surface. The flowers are tubular, bilateral, and epigynous, measuring 5 cm in length and predominantly white with pink hues. They lack stipules, have small bracts, and are clustered on spikes. Each fruit, classified as a capsule, is oblong, turning from green to red as it matures and splits open into three parts. Each capsule contains one ellipsoid seed with a dark brown, impermeable coat, and no visible hilum. The seed has two cotyledons and a radicle.

Alpinia officinarum exhibits a distinct combination of root, stem, rhizome, leaf, flower, fruit, and seed characteristics, making it well-suited for its ecological niche and various uses in traditional medicine and cuisine.

In this article you will learn about the morphology of Alpinia officinarum by analyzing the main parts of this plant and their characteristics.

## Roots



Aplinia officinarum has a fibrous root system, consisting of a main taproot and lateral fibrous roots. These roots display dichotomous branching, and their tips are acute in shape.

The root density of Aplinia officinarum is approximately 1.5 grams per cubic centimeter. In terms of depth, the main taproot typically extends to a depth of 20-30 centimeters (approximately 7.9-11.8 inches), while the lateral roots vary in depth.

The diameter of the main taproot measures about 1.5 centimeters (0.59 inches), whereas the lateral roots are comparatively finer, with a diameter of approximately 0.2 centimeters (0.08 inches). The main taproot reaches a length of 25-30 centimeters (9.8-11.8 inches), while the lateral roots can have varied lengths.

The color of Aplinia officinarum roots is generally light brown, and their texture is smooth to the touch. These roots have a hairy surface, with the presence of root hairs facilitating nutrient and water absorption.

Aplinia officinarum roots do not develop nodules. These morphological characteristics collectively define the root system of Aplinia officinarum, highlighting its adaptability and functional traits for nutrient uptake and anchorage in its habitat.

The following table shows in detail the morphological characteristics of Alpinia officinarum roots.

| Characteristic | Description |
| --- | --- |
| Root System Type  | Fibrous |
| Root Structure  | Taproot (main), fibrous (lateral) |
| Root Branching  | Dichotomous |
| Root Tips  | Acute |
| Root Density  | Approximately 1.5 g/cm3 |
| Root Depth  | 20-30 cm (7.9-11.8 inches) |
| Root Diameter  | 1.5 cm (0.59 inches) |
| Root Length  | 25-30 cm (9.8-11.8 inches) |
| Root Color  | Light brown |
| Root Surface Texture  | Smooth |
| Root Nodules  | Absent |
| Root Hairs  | Present |

![none](/images/alpinia-officinarum-botany-morphology-roots.jpg "none")

## Stems



Alpinia officinarum gas herbaceous stems, lacking the woody nature found in some plants. They display an alternating arrangement along the stem, with cylindrical shapes that are slightly ribbed. In terms of size, the stems can grow to heights ranging from 1 to 3 meters (3.28 to 9.84 feet), with a diameter of approximately 1 to 3 centimeters (0.39 to 1.18 inches).

The color of these stems is predominantly green, often marked by reddish or brown patches. When touched, the stems have a smooth texture, sometimes featuring fine hairs. Their surface is notably glossy and possesses a waxy quality.

Stem nodes on Alpinia officinarum are distinct and serve as points where leaves and branches originate. The internodes, which are the spaces between these nodes, measure about 10 to 30 centimeters (3.94 to 11.81 inches) in length.

The following table shows in detail the morphological characteristics of Alpinia officinarum stems.

| Characteristic | Description |
| --- | --- |
| Stem Type  | Herbaceous, non-woody |
| Stem Arrangement  | Alternating |
| Stem Shape  | Cylindrical, slightly ribbed |
| Stem Height  | 1-3 meters (3.28-9.84 feet) |
| Stem Diameter  | 1-3 centimeters (0.39-1.18 inches) |
| Stem Color  | Green with reddish or brown patches |
| Stem Texture  | Smooth, with occasional fine hairs |
| Stem Surface  | Glossy, waxy |
| Stem Nodes  | Distinct nodes with leaves and branches |
| Stem Internodes  | Approximately 10-30 centimeters (3.94-11.81 inches) |

![none](/images/alpinia-officinarum-botany-morphology-stems.jpg "none")

## Leaves



Alpinia officinarum has leaves that are relatively simple, with an oblong shape, measuring approximately 10 centimeters in length and 3 centimeters in width. The thickness of these leaves is around 0.2 centimeters. They are primarily green in color and have a smooth, glossy texture, which imparts a vibrant appearance.

The leaves of Alpinia officinarum are arranged alternately along the stem and oriented vertically. The apex of these leaves is acute, while the base is cuneate, giving them a distinctive overall appearance. The leaf margin is entire, and the venation is parallel, further enhancing their aesthetic appeal.

In terms of leaf structure, Alpinia officinarum leaves are simple, with a prominent midrib running along the center. They attach to the stem through petioles that are approximately 2 centimeters in length. Interestingly, stipules, a common feature in many plant species, are absent in Alpinia officinarum leaves.

The following table shows in detail the morphological characteristics of Alpinia officinarum leaves.

| Characteristic | Description |
| --- | --- |
| Leaf Complexity  | Simple |
| Leaf Shape  | Oblong (10 cm x 3 cm) |
| Leaf Arrangement  | Alternate |
| Leaf Orientation  | Vertical |
| Leaf Length  | 10 cm (3.94 inches) |
| Leaf Width  | 3 cm (1.18 inches) |
| Leaf Thickness  | 0.2 cm (0.08 inches) |
| Leaf Color  | Green |
| Leaf Texture  | Smooth |
| Leaf Surface  | Glossy |
| Leaf Apex  | Acute |
| Leaf Base  | Cuneate |
| Leaf Margin  | Entire |
| Leaf Venation  | Parallel |
| Leaf Lamina  | Simple |
| Leaf Midrib  | Prominent |
| Leaf Attachment  | Petiolate |
| Leaf Petiole  | 2 cm (0.79 inches) |
| Leaf Stipule | Absent |

![none](/images/alpinia-officinarum-botany-morphology-leaves.jpg "none")

## Flowers



Alpinia officinarum has a raceme inflorescence type, which is a spike-like arrangement of flowers along a central stem. These flowers possess a relatively simple structure with a tubular shape. Notably, they exhibit bilateral symmetry, meaning the flower can be divided into two equal halves in only one plane.

The flowers of Alpinia officinarum are typically epigynous, attached above the ovary. They are approximately 5 cm (1.97 inches) in length and 2 cm (0.79 inches) in width. These flowers are predominantly white with delicate pink hues, giving them an attractive aesthetic.

Upon touch, the flowers are smooth and glabrous, lacking hair or trichomes. Each flower comprises six floral parts, consisting of three sepals and three petals. The sepals are green and measure around 1 cm (0.39 inches) in length, while the petals are white with pink hues and are about 2 cm (0.79 inches) long. Together, the sepals and petals form the perianth, the protective outer covering of the flower.

The reproductive structures of these flowers include one fertile stamen and two sterile staminodes, along with a single pistil. The ovary is located beneath the other floral parts, making it an inferior ovary with three lobes. The style extends from the ovary and terminates with a lobed stigma at its tip.

Notably, Alpinia officinarum flowers do not have stipules, and their bracts are small and inconspicuous. The flowers are borne on a peduncle that varies in length, supporting the inflorescence, while each individual flower is attached to the inflorescence through a short pedicel.

The following table shows in detail the morphological characteristics of Alpinia officinarum flowers.

| Characteristic | Description |
| --- | --- |
| Flower Type  | Raceme, a spike-like inflorescence |
| Inflorescence Type  | Simple |
| Flower Complexity  | Zygomorphic (bilateral symmetry) |
| Flower Symmetry  | Tubular |
| Flower Structure   | Epigynous (attached above the ovary) |
| Flower Attachment  | Tubular |
| Flower Shape  | 5 cm (1.97 inches) |
| Flower Length  | 2 cm (0.79 inches) |
| Flower Width  | White with pink hues |
| Flower Thickness  | Smooth |
| Flower Color  | Glabrous (hairless) |
| Number Of Floral Parts  | 6 (3 sepals, 3 petals) |
| Flower Sepals   | 3, green, 1 cm (0.39 inches) long |
| Flower Petals   | 3, white with pink hues, 2 cm (0.79 inches) long |
| Flower Perianth  | Present (collective term for sepals and petals) |
| Flower Stamen  | 1 fertile stamen and 2 sterile staminodes |
| Flower Pistil  | 1, with an inferior ovary |
| Flower Ovary  | Inferior, 3-lobed |
| Flower Style  | 1, extending from the ovary |
| Flower Stigma  | Lobed, at the tip of the style |
| Flower Stipules  | Absent |
| Flower Bracts  | Small, inconspicuous |
| Flower Peduncle  | Length varies, supports the inflorescence |
| Flowers Type | Short, supports individual flowers in the inflorescence |

## Fruits



Alpinia officinarum has fruits that are classified as capsules, and these capsules are clustered on spikes, distributed along the plant's inflorescence. Each fruit is attached to the plant via a pedicel, the slender stalk that connects it to the stem.

In terms of shape, the fruits of Alpinia officinarum are oblong, measuring approximately 2.5 cm (0.98 inches) in length, 0.8 cm (0.31 inches) in width, and 0.5 cm (0.20 inches) in thickness. These fruits undergo a color transformation during their development. When they are unripe, they present a green hue, but as they mature, they turn a striking red.

Upon touch, these fruits have a smooth, leathery texture with a glabrous, hairless surface. Notably, they lack prominent markings. One distinct feature of these fruits is their dehiscence, which occurs in three parts. When they reach maturity, the capsules split open, releasing their seeds.

The following table shows in detail the morphological characteristics of Alpinia officinarum fruits.

| Characteristic | Description |
| --- | --- |
| Fruit Type  | Capsule |
| Fruit Arrangement  | Spikes, clustered |
| Fruit Distribution  | Axillary, along the inflorescence |
| Attachment | Pedicel |
| Fruit Shape  | Oblong |
| Fruit Length  | 2.5 cm (0.98 inches) |
| Fruit Width  | 0.8 cm (0.31 inches) |
| Fruit Thickness  | 0.5 cm (0.20 inches) |
| Fruit Color  | Red when mature, green when unripe |
| Fruit Texture  | Smooth, leathery surface |
| Fruit Surface  | Glabrous (hairless) |
| Fruit Marking  | Absence of prominent markings |
| Fruit Dehiscence | Three-parted, splits open to release seeds |

## Seeds



Alpinia officinarum has seeds that are of the endospermic type, which means they contain endosperm tissue. Typically, a single seed is found within each fruit, and the seed has an ellipsoid shape, measuring about 2.5 cm in length, 1 cm in width, and 0.5 cm in thickness (approximately 0.98 inches long, 0.39 inches wide, and 0.2 inches thick).

Notably, these seeds lack any appendages. They are hard and have a dark brown seed coat that is smooth, shiny, and thin. The seed coat is impermeable, meaning it does not allow the passage of water or gases. One distinctive feature is the absence of a visible scar known as the hilum.

The embryo within the seed is small and centrally located, while the cotyledons are present as two dicotyledonous structures. Additionally, each seed contains a radicle, which emerges from one end of the seed.

The following table shows in detail the morphological characteristics of Alpinia officinarum seeds.

| Characteristic | Description |
| --- | --- |
| Seed Type  | Endospermic (endosperm present) |
| Seed Arrangement  | Single (solitary seed per fruit) |
| Seed Shape  | Ellipsoid (2.5 cm long, 1 cm wide, 0.5 cm thick) |
| Seed Appendages  | None |
| Seed Number  | Usually one seed per fruit |
| Seed Length  | Approximately 2.5 cm (0.98 inches) |
| Seed Width  | Approximately 1 cm (0.39 inches) |
| Seed Thickness  | Approximately 0.5 cm (0.2 inches) |
| Seed Hardness  | Hard |
| Seed Coat Color  | Dark brown |
| Seed Coat Surface  | Smooth |
| Seed Coat Texture  | Shiny |
| Seed Coat Thickness  | Thin |
| Seed Coat Permeability  | Impermeable |
| Seed Hilum  | Absent (no visible scar) |
| Seed Embryo  | Small, centrally located |
| Seed Cotyledon | Two cotyledons (dicotyledonous) |
| Seed Radicle | Present, emerging from one end of the seed |

