# Barosma betulina morphology

![Barosma betulina morphology](/images/barosma-betulina-botany-morphology.jpg "Barosma betulina morphology")

Barosma betulina, a plant with fibrous roots, features a primary taproot and sparse lateral branching. These roots are shallow, reaching 10-20 cm in depth, with a diameter of 0.2-0.5 cm and a length of 15-30 cm. They are brown to reddish-brown, with smooth surfaces covered in fine root hairs, aiding in nutrient and water absorption. Notably, these roots are densely packed, with approximately 5,000 roots per square meter.

The herbaceous stems of Barosma betulina are arranged oppositely and typically grow to heights of 10-20 cm with diameters between 0.2 and 0.5 cm. They are green but may change with maturity, featuring a pubescent texture. Nodes are evenly spaced along the stem, and internodes are 2-5 cm in length.

The plant's leaves are elliptical, thin, and dark green, with smooth upper surfaces and punctate lower surfaces. They are arranged oppositely, have acute tips, cuneate bases, and smooth margins. The prominent midrib follows pinnate venation, and the leaves attach to the stem via short petioles. Stipules are absent.

Barosma betulina's flowers form a racemose inflorescence with actinomorphic symmetry. They are small, bell-shaped, pale pink, and have a glabrous texture. Each flower consists of five floral parts, including sepals and petals fused at the base, five stamens alternating with petals, and a single pistil with a filiform style and capitate stigma. Stipules are absent, and peduncles are typically absent as well.

The drupe fruits of Barosma betulina are solitary, ovate, dark brown to black, and attach via axial stalks. They are smooth, glossy, and indehiscent, meaning they don't split open naturally.

The dicotyledonous seeds are solitary, small, oval, and hard, with brown, impermeable seed coats. Each fruit typically contains one seed with two fleshy cotyledons and a tiny radicle.

In this article you will learn about the morphology of Barosma betulina by analyzing the main parts of this plant and their characteristics.

## Roots



Barosma betulina's root system type is fibrous, which means it consists of numerous thin roots extending from the base of the plant. The root structure is primarily taproot, with one dominant primary root that extends deeper into the soil. The branching of the roots is minimal, with few lateral branches.

These roots are relatively shallow, typically extending to a depth of 10-20 cm in the soil. They are relatively thin, with a diameter ranging from 0.2 to 0.5 cm. In terms of length, the roots of Barosma betulina reach a moderate 15-30 cm.

The color of these roots is typically brown to reddish-brown, and their surface texture is smooth, often adorned with fine root hairs measuring 0.1-0.2 mm in length. These fine root hairs play a vital role in nutrient and water absorption.

One distinctive characteristic of Barosma betulina's roots is their high density. On average, there are approximately 5,000 roots per square meter, allowing for efficient resource uptake from the surrounding soil.

Notably, this plant does not form root nodules, which are often associated with nitrogen-fixing plants.

The following table shows in detail the morphological characteristics of Barosma betulina roots.

| Characteristic | Description |
| --- | --- |
| Root System Type  | Fibrous |
| Root Structure  | Taproot |
| Root Branching  | Minimal lateral branching |
| Root Tips  | Single primary root tip |
| Root Density  | High density (approximately 5,000 roots/m2) |
| Root Depth  | Shallow (10-20 cm) |
| Root Diameter  | Thin (0.2-0.5 cm) |
| Root Length  | 15-30 cm |
| Root Color  | Brown to reddish-brown |
| Root Surface Texture  | Smooth with fine root hairs |
| Root Nodules  | Absent |
| Root Hairs  | Abundant and fine, 0.1-0.2 mm in length |

![none](/images/barosma-betulina-botany-morphology-roots.jpg "none")

## Stems



The stems of Barosma betulina are herbaceous (or occasionally woody) and the arrangement is opposite, meaning that leaves and branches grow in pairs along the stem. The stem itself has a cylindrical (or prismatic) shape, with a typical height ranging from 10 to 20 centimeters (3.94 to 7.87 inches). Its diameter falls between 0.2 and 0.5 centimeters (0.08 to 0.2 inches). These measurements contribute to the compact nature of the plant.

The color of Barosma betulina's stem is predominantly green, although this may vary with maturity. Young stems often appear greener, while older ones may develop a slightly different hue. The stem texture is characterized by its pubescent nature, meaning that it is covered with fine hairs. This texture gives the stem a unique tactile quality.

The stem surface is generally smooth, with only minor variations, which could be slightly rough in some cases. Nodes along the stem are present and occur at regular intervals, adding structure and support to the plant. Stem internodes, the segments of the stem between nodes, are typically about 2 to 5 centimeters (0.79 to 1.97 inches) in length.

The following table shows in detail the morphological characteristics of Barosma betulina stems.

| Characteristic | Description |
| --- | --- |
| Stem Type  | Herbaceous (or woody) |
| Stem Arrangement  | Opposite |
| Stem Shape  | Cylindrical (or prismatic) |
| Stem Height  | 10-20 cm (3.94-7.87 inches) |
| Stem Diameter  | 0.2-0.5 cm (0.08-0.2 inches) |
| Stem Color  | Green (varies with maturity) |
| Stem Texture  | Pubescent (covered with fine hairs) |
| Stem Surface  | Smooth (or slightly rough) |
| Stem Nodes  | Present, spaced at regular intervals |
| Stem Internodes  | Approximately 2-5 cm (0.79-1.97 inches) in length |

![none](/images/barosma-betulina-botany-morphology-stems.jpg "none")

## Leaves



The leaves of Barosma betulina are simple, with an elliptical shape that typically measures 1.5 to 3.5 centimeters in length and 0.5 to 1.5 centimeters in width. These leaves are rather thin, with a thickness of approximately 0.2 millimeters, and possess a dark green coloration. Their upper surface is smooth and glossy, while the lower surface showcases small glands, giving it a punctate texture.

In terms of arrangement, Barosma betulina leaves are opposite, with an alternating and mostly horizontal orientation along the stem. The apex of these leaves is acute, meaning they come to a pointed tip, while the base is cuneate, exhibiting a wedge-shaped form. The leaf margins remain entirely smooth and unbroken, contributing to their overall appearance.

A distinctive feature of these leaves is their pinnate venation, characterized by a single prominent midrib along the center, from which smaller veins branch off. The leaf lamina, or the overall shape of the leaf, is ovate or egg-shaped. The midrib, which is notably prominent, adds to the leaf's structure and support.

Leaf attachment in Barosma betulina is petiolate, meaning the leaves are attached to the stem via a petiole. The petiole is relatively short, measuring about 1 to 2 millimeters in length. One notable absence in this plant's leaves is stipules; they do not have the small leaf-like structures, stipules, that are common in some other plants.

The following table shows in detail the morphological characteristics of Barosma betulina leaves.

| Characteristic | Description |
| --- | --- |
| Leaf Complexity  | Simple |
| Leaf Shape  | Elliptical (1.5-3.5 cm long, 0.5-1.5 cm wide) |
| Leaf Arrangement  | Opposite |
| Leaf Orientation  | Alternating, horizontal |
| Leaf Length  | 1.5-3.5 cm (0.6-1.4 inches) |
| Leaf Width  | 0.5-1.5 cm (0.2-0.6 inches) |
| Leaf Thickness  | Thin, approximately 0.2 mm (0.008 inches) |
| Leaf Color  | Dark green |
| Leaf Texture  | Smooth and waxy (glossy) |
| Leaf Surface  | Upper surface smooth, lower surface with small glands (punctate) |
| Leaf Apex  | Acute (pointed) |
| Leaf Base  | Cuneate (wedge-shaped) |
| Leaf Margin  | Entire (smooth and unbroken) |
| Leaf Venation  | Pinnate (single main vein with smaller veins branching off) |
| Leaf Lamina  | Ovate (egg-shaped) |
| Leaf Midrib  | Prominent along the center of the leaf |
| Leaf Attachment  | Petiolate (attached to the stem via a petiole) |
| Leaf Petiole  | Short, 1-2 mm (0.04-0.08 inches) in length |
| Leaf Stipule | Absent (not present) |

## Flowers



The inflorescence of Barosma betulina is racemose, meaning that its flowers are arranged in a simple, unbranched cluster. Each individual flower exhibits a simple structure and follows an actinomorphic (radial) symmetry, giving it a balanced appearance.

The flowers are small, with a bell-shaped structure measuring approximately 0.5 cm (0.2 inches) in length and 0.3 cm (0.12 inches) in width. They showcase a pale pink color and a smooth, glabrous texture. The tepals, which make up the perianth, form a tubular shape.

These buchu flowers consist of five floral parts, including sepals and petals that are fused at their base. In addition to the fused floral parts, each flower possesses five stamens that alternate with the petals, as well as a single, terminal pistil with a superior ovary. The style of the pistil is filiform and measures about 0.2 cm (0.08 inches) in length, while the stigma takes on a capitate (round) form.

Notably, Barosma betulina lacks stipules, and its bracts, when present, are tiny and inconspicuous, typically located near the base of the pedicel. Speaking of pedicels, they are relatively short, measuring about 0.1 cm (0.04 inches) in length if present. However, the peduncle is typically absent, indicating that the flowers are axillary in attachment.

The following table shows in detail the morphological characteristics of Barosma betulina flowers.

| Characteristic | Description |
| --- | --- |
| Flower Type  | Racemose |
| Inflorescence Type  | Simple |
| Flower Complexity  | Actinomorphic (radial) |
| Flower Symmetry  | Tubular |
| Flower Structure   | Axillary |
| Flower Attachment  | Bell-shaped |
| Flower Shape  | 0.5 cm (0.2 inches) |
| Flower Length  | 0.3 cm (0.12 inches) |
| Flower Width  | Pale pink |
| Flower Thickness  | Smooth |
| Flower Color  | Glabrous (hairless) |
| Number Of Floral Parts  | 5 |
| Flower Sepals   | 5, fused at base |
| Flower Petals   | 5, fused at base |
| Flower Perianth  | Tepals forming a tube |
| Flower Stamen  | 5, alternate with petals |
| Flower Pistil  | 1, terminal |
| Flower Ovary  | Superior, 1-carpelled |
| Flower Style  | 1, filiform, about 0.2 cm (0.08 inches) long |
| Flower Stigma  | Capitate (round) |
| Flower Stipules  | Absent |
| Flower Bracts  | Tiny, inconspicuous, near base of pedicel |
| Flower Peduncle  | Absent |
| Flowers Type | Short, about 0.1 cm (0.04 inches) long, if present |

## Fruits



The fruits of Barosma betulina are drupes that are solitary in arrangement and can be found distributed throughout the entire plant. They attach to the plant via a stalk, known as axial attachment.

In terms of their shape, Barosma betulina fruits are typically ovate, with dimensions of approximately 2.5 cm in length and 1.5 cm in width. They are dark brown to black in color, giving them a distinct appearance.

The texture of these fruits is smooth, and their surface has a glossy finish. Unlike some other fruit types, they do not display any distinct markings. Importantly, these drupes are indehiscent, meaning they do not split open to release their seeds naturally.

The following table shows in detail the morphological characteristics of Barosma betulina fruits.

| Characteristic | Description |
| --- | --- |
| Fruit Type  | Drupe |
| Fruit Arrangement  | Solitary |
| Fruit Distribution  | Throughout the plant |
| Attachment | Axial (stalked) |
| Fruit Shape  | Ovate (2.5 cm long, 1.5 cm wide) |
| Fruit Length  | 2.5 cm (0.98 inches) |
| Fruit Width  | 1.5 cm (0.59 inches) |
| Fruit Thickness  | N/a (not applicable, as drupes are typically not measured for thickness) |
| Fruit Color  | Dark brown to black |
| Fruit Texture  | Smooth |
| Fruit Surface  | Glossy |
| Fruit Marking  | None (no distinct markings) |
| Fruit Dehiscence | Indehiscent (does not split open) |

## Seeds



The seeds of Barosma betulina are dicotyledonous and they are typically arranged as solitary units within the fruit. They have an oval or ellipsoid shape, with no appendages. Each fruit usually contains one seed.

The seeds of Barosma betulina are relatively small, measuring approximately 2.5 mm in length, 1.5 mm in width, and 1 mm in thickness. They are characterized by their hardness, presenting a woody texture. The seed coat is brown, smooth, and exhibits a shiny, glossy surface. Its thickness is about 0.2 mm, making it impermeable to water.

The hilum, a small scar where the seed is attached to the fruit, is located at one end of the seed. Within the seed, a tiny, rudimentary embryo can be found. Furthermore, the seed contains two fleshy and thick cotyledons, essential for the initial growth of the plant. Additionally, a small radicle, which is the embryonic root, emerges from the hilum scar.

The following table shows in detail the morphological characteristics of Barosma betulina seeds.

| Characteristic | Description |
| --- | --- |
| Seed Type  | Dicotyledonous |
| Seed Arrangement  | Solitary |
| Seed Shape  | Oval, ellipsoid |
| Seed Appendages  | None |
| Seed Number  | Typically one per fruit |
| Seed Length  | 2.5 mm (0.098 inches) |
| Seed Width  | 1.5 mm (0.059 inches) |
| Seed Thickness  | 1 mm (0.039 inches) |
| Seed Hardness  | Hard, woody |
| Seed Coat Color  | Brown |
| Seed Coat Surface  | Smooth |
| Seed Coat Texture  | Shiny, glossy |
| Seed Coat Thickness  | 0.2 mm (0.008 inches) |
| Seed Coat Permeability  | Impermeable to water |
| Seed Hilum  | Located at one end, small scar |
| Seed Embryo  | Present, tiny, rudimentary |
| Seed Cotyledon | Two, fleshy, thick cotyledons |
| Seed Radicle | Present, small, emerging from hilum scar |

